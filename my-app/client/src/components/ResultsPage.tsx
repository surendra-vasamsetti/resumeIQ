...
import React, { useState, useEffect } from 'react';
import { ArrowLeft, Download, Share2, TrendingUp, AlertCircle, CheckCircle, Target, Sparkles, Award, Zap, RefreshCw } from 'lucide-react';
import ThemeToggle from './ThemeToggle';
import { analyzeResume, testAPI, AnalysisResponse } from '../services/api';
  ...
  useEffect(() => {
    const checkHealth = async () => {
      try {
        ...
        // Handle case where testAPI returns undefined or null
        if (!healthResult) {
          console.error('API health check returned undefined');
          setApiHealthy(false);
          return;
        }
        
        // Check if healthResult has the expected structure
        if (typeof healthResult === 'object' && 'healthy' in healthResult) {
          setApiHealthy(healthResult.healthy);
          if (!healthResult.healthy) {
            console.error('API health check failed:', healthResult.error || 'Unknown error');
          }
        } else {
          console.error('API health check returned unexpected format:', healthResult);
          setApiHealthy(false);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      try {
        ...
        // Animate scores - with safety checks
        if (response && response.analysis && response.analysis.scores) {
          const targetScores = {
            overall: response.analysis.overallScore || 0,
            impact: response.analysis.scores.impact || 0,
            brevity: response.analysis.scores.brevity || 0,
            style: response.analysis.scores.style || 0,
            structure: response.analysis.scores.structure || 0,
            skills: response.analysis.scores.skills || 0,
            atsCompatibility: response.analysis.scores.atsCompatibility || 0
          };
    ...
    if (file && role && analysisType) {
      fetchAnalysis();
    }
  }, [file, role, analysisType, jobDescription, retryCount]);
          ...
          {apiHealthy === false && (
            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4 mb-4 max-w-md mx-auto">
              <div className="flex items-center text-red-700 dark:text-red-400">
                ...
                <span className="text-sm">API connection issue detected</span>
              </div>
            </div>
          )}
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-purple-900 flex items-center justify-center transition-colors duration-500">
        <div className="text-center animate-fade-in-up max-w-lg mx-auto px-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mb-6 text-left">
            <h3 className="font-semibold text-gray-900 dark:text-white mb-2">Debug Information:</h3>
            <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
              <p>• File: {file.name} ({(file.size / 1024).toFixed(2)} KB)</p>
              <p>• Type: {file.type}</p>
              <p>• Role: {role}</p>
              <p>• Analysis Type: {analysisType}</p>
              <p>• API Status: {apiHealthy === null ? 'Checking...' : apiHealthy ? 'Healthy' : 'Unhealthy'}</p>
          ...
          <div className="space-y-3">
            <button
              onClick={handleRetry}
              className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-full font-semibold hover:from-blue-700 hover:to-purple-700 transform hover:scale-105 transition-all duration-300 flex items-center justify-center mx-auto"
            >
            ...
            <button
              onClick={onBack}
              className="bg-gray-600 text-white px-6 py-3 rounded-full font-semibold hover:bg-gray-700 transform hover:scale-105 transition-all duration-300 flex items-center justify-center mx-auto"
            >
          ...
          <div className="mt-6 text-sm text-gray-500 dark:text-gray-400">
            ...
            <ul className="mt-2 space-y-1">
              ...
              <li>• Try a different file format (PDF, DOC, DOCX)</li>
            </ul>
          </div>
        </div>
      </div>
